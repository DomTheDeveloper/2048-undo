...
/-- One pending tile, consumed exactly once; every recursive call drops one input.
A produced merged tile is emitted, never returned to the pending slot. -/
def mergeAux (pending : Option Nat) (xs : List Nat) : List Nat :=
  match xs with
  | [] => match pending with | none => [] | some a => [a]
  | b :: rest =>
    match pending with
    | none => mergeAux (some b) rest
    | some a =>
      if a = b then (a+b) :: mergeAux none rest
      else a :: mergeAux (some b) rest
termination_by structural xs

def merge (xs : List Nat) : List Nat := mergeAux none xs

@[simp] theorem merge_nil : merge [] = [] := rfl
@[simp] theorem merge_single (a : Nat) : merge [a] = [a] := rfl
@[simp] theorem merge_pair (a b : Nat) (rest : List Nat) :
    merge (a :: b :: rest) =
      if a = b then (a+b) :: merge rest else a :: merge (b :: rest) := rfl
...
theorem merge_heavy (xs : List Nat) (k : Nat) :
  ...
  cases xs with
  | nil => simp [merge_nil, merge_single, merge_pair, ite_true, heavyList]
  | cons a xs =>
    cases xs with
    | nil => simpa [merge_nil, merge_single, merge_pair, ite_true, heavyList] using heavyTile_mono_double k a
    | cons b rest =>
      by_cases h : a = b
      · subst b
        have ih := merge_heavy rest k
        have hh := Nat.add_le_add_left ih (heavyTile k a + heavyTile k a)
        simpa [merge_nil, merge_single, merge_pair, ite_true, heavyList, heavyTile_double, Nat.add_assoc] using hh
      · have ih := merge_heavy (b :: rest) k
        have ha := heavyTile_mono_double k a
        simp only [merge_nil, merge_single, merge_pair, ite_true, if_neg h, heavyList] at *
        omega
termination_by xs.length

theorem merge_length (xs : List Nat) : (merge xs).length ≤ xs.length := by
  cases xs with
  | nil => simp only [merge_nil, merge_single, List.length_nil, List.length_cons]; omega
  | cons a xs =>
    cases xs with
    | nil => simp only [merge_nil, merge_single, List.length_nil, List.length_cons]; omega
    | cons b rest =>
      ...
      · have ih := merge_length rest
        simp only [merge_nil, merge_single, merge_pair, ite_true, if_pos h, List.length_cons] at *
        omega
      · have ih := merge_length (b :: rest)
        simp only [merge_nil, merge_single, merge_pair, ite_true, if_neg h, List.length_cons] at *
        omega
termination_by xs.length
